...
Issues are tracked as local markdown files under `.scratch/<feature>/`. See [agent issue-tracker docs](docs/agents/issue-tracker.md).

### Triage labels

Default label vocabulary (`needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, `wontfix`). See [agent triage-label docs](docs/agents/triage-labels.md).

### Domain docs

Single-context layout — one `CONTEXT.md` + `docs/adr/` at the repo root. See [agent domain docs](docs/agents/domain.md).
