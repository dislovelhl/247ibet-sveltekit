...
async function auditPage(page: { path: string }, baseUrl: string): Promise<SeoPageReport> {
  ...
  return { path: page.path, title, description, canonical, h1Count, jsonLdCount, issues };
}
...
export async function seoAuditWorkflow(): Promise<SeoAuditResult> {
  ...
  const summary = {
    ...
    warnCount: pages.reduce((n, p) => n + p.issues.filter((i) => i.severity === 'warn').length, 0),
  };
