...
const DEFAULT_SCHEMA_OUTPUT_DIR = '.workflow-data/schema';

async function writeSchemaFile(slug: string, schema: FAQPageSchema): Promise<void> {
  'use step';
  const { mkdir, writeFile } = await import('fs/promises');
  const { resolve } = await import('path');
  const outputDir = process.env.AEO_SCHEMA_OUTPUT_DIR?.trim() || DEFAULT_SCHEMA_OUTPUT_DIR;
  const schemaDir = resolve(outputDir);
  await mkdir(schemaDir, { recursive: true });
  await writeFile(resolve(schemaDir, `${slug}-faq.json`), JSON.stringify(schema, null, 2), 'utf-8');
}
