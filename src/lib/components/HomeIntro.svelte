<section class="max-w-5xl space-y-5">
  <div>
    <div class="inline-flex items-center justify-center gap-2 rounded-full border border-slate-blue/20 bg-slate-blue/10 px-4 py-1.5 text-xs font-mono font-bold uppercase tracking-widest text-slate-blue shadow-[0_0_15px_rgba(74,158,191,0.15)]">
      Canada's Regulated iGaming Research Platform
    </div>
  </div>

  <h2 class="max-w-4xl font-display text-[clamp(2.5rem,5vw,4rem)] font-light leading-[1.02] tracking-[-0.035em] text-text-primary">
    Regulated Canadian iGaming, <span class="text-gradient-slate">clearly explained.</span>
  </h2>

  <p class="max-w-3xl text-lg leading-relaxed text-text-body md:text-xl">
    Independent verification of payout speed, licensing registry, and player protection protocols for Ontario and Alberta residents.
  </p>

  <div class="flex flex-wrap items-center gap-3">
    <a
      href="/best-online-casinos-canada"
      class="rounded-md btn-glossy-gold px-8 py-3.5 text-sm uppercase tracking-widest font-black transition-all hover:scale-[1.02]"
    >
      Analysis Lobby
    </a>
    <a
      href="/editorial-policy"
      class="rounded-md border border-white/15 px-8 py-3.5 text-sm uppercase tracking-widest font-bold text-text-primary transition-all hover:bg-white/5 hover:border-white/30"
    >
      Methodology Review
    </a>
  </div>

  <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-text-tertiary">
    <span>Last reviewed March 2026</span>
    <span aria-hidden="true">·</span>
    <span>Official AGCO/iGaming Ontario sources</span>
    <span aria-hidden="true">·</span>
    <span>KYC-complete Interac test data</span>
    <span aria-hidden="true">·</span>
    <span>Responsible gambling tools reviewed</span>
  </div>
</section>
