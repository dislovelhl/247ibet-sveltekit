<script lang="ts">
  import { ArrowRight, BadgeCheck, CircleDollarSign, ShieldCheck } from 'lucide-svelte';
  ...
  <div class="relative overflow-hidden rounded-[1.75rem] border border-navy-border luxury-card p-6 sm:rounded-3xl sm:p-8 md:p-12">
    <div class="relative z-10 grid gap-6 sm:gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
      <div>
        ...
        <p class="page-hero-subtitle mt-4 max-w-2xl text-sm leading-relaxed sm:mt-6 sm:text-base md:text-lg">
          Access casino games, live sports betting, and Interac deposits in one place. Join 247iBET for faster funding, simpler withdrawals, and a mobile-ready account built around Canadian play.
        </p>
        ...
        <div class="mt-8 flex flex-col gap-3 text-xs font-bold uppercase tracking-wider text-text-body sm:mt-10 sm:flex-row sm:flex-wrap sm:gap-x-6 sm:gap-y-3">
          {#each trustPoints as point}
            <span class="inline-flex items-start gap-2 sm:items-center">
              <ShieldCheck class="h-4 w-4 text-success drop-shadow-[0_0_8px_rgba(34,197,94,0.4)]" aria-hidden="true" />
              {point}
            </span>
          {/each}
        </div>
      ...
      <div class="rounded-[1.5rem] border border-white/5 p-5 sm:rounded-3xl sm:p-8 md:p-10">
        ...
        <div class="space-y-3">
          ...
          <a
            href="/fast-payouts"
            class="flex items-center justify-between rounded-xl border border-white/5 bg-white/[0.02] px-5 py-4 text-sm font-bold text-white transition-all hover:border-success/40 hover:bg-white/[0.05] group/item"
          >
            <span class="inline-flex items-start gap-2 sm:items-center">
              <CircleDollarSign class="h-4 w-4 text-success" aria-hidden="true" />
              Fast Withdrawals
            </span>
            <ArrowRight class="h-4 w-4 text-success transition-transform duration-200 group-hover/item:translate-x-1" aria-hidden="true" />
          </a>
          <a
            href="/responsible-gambling"
            class="flex items-center justify-between rounded-xl border border-white/5 bg-white/[0.02] px-5 py-4 text-sm font-bold text-white transition-all hover:border-white/20 hover:bg-white/[0.05] group/item"
          >
            <span>Safety Protocols</span>
            <ArrowRight class="h-4 w-4 text-text-body transition-transform duration-200 group-hover/item:translate-x-1" aria-hidden="true" />
          </a>
        </div>
